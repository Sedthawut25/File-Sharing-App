...
const FileCard = ({file, onDelete, onTogglePublic, onDownload, onShareLink}) => {
    ...
    const formatFileSize = (bytes) => {
        ...
        else if (bytes < 104857) return (bytes / 1024).toFixed(1) + 'KB';
    ...
    return (
        <div 
            ...
            className="relative group overflow-hidden rounded-xl bg-white shadow-md hover:shadow-lg transition-all duration-300 border-gray-100">
            {/* File preview area */}
            <div className="h-32 bg-gradient-to-br from-red-50 to-indigo-50 flex items-center justify-center p-4">
                {getFileIcon(file)}
            </div>
            ...
            <div className="p-4">
                <div className="flex justify-between items-start">
                    <div className="overflow-hidden">
                        <h3 title={file.name} className="font-medium text-gray-900"> 
                            {file.name}
                        </h3>
                ...
                <div className="flex gap-3 w-full justify-center ">
                    {file.isPublic && (
                        <button 
                            ...
                            title="Share Link"
                            className="p-2 cursor-pointer bg-white/90 rounded-full hover:bg-white transition-colors text-red-500 hover:text-red-600">
                            <Copy size={18}/>
                        </button>
                    )}
